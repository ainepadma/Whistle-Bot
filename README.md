...
- 首次启动显示四步使用教学，介绍桌宠交互、日程卡片、专注计时与右键设置；之后可从“设置 → 使用说明”再次打开
...
- 新增首次启动使用教学，并在设置中提供长期保留的“使用说明”入口；教学主题色跟随当前小鹞配色
- 日程深度整合：提供行动、今天、日历、管理四张桌面卡片；未固定时原位切换，固定后可并存管理
- 卡片尺寸统一：今天／行动为 400 × 480，日历／管理为 960 × 680；旧布局自动迁移，行动卡片不再随内容伸缩
...
- 今天、行动统一为 400 × 480；日历、管理统一为 960 × 680（逻辑像素，随 Windows DPI 缩放，超出屏幕工作区时自动收缩）。
...
教学成功显示后写入独立的一次性标记，不受颜色、尺寸配置更新影响。需要回看时，右键小鹞并打开“设置 → 使用说明”。
