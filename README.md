...
- 行动／今天从相同的 320 × 320 方形卡片开始，有内容时自动向下增高；日历为 940 × 660，管理为 720 × 380，所有卡片均可拖动边缘调整宽高
- 日程工作流：专注番茄钟、待办、即将开始和日历互通，支持从卡片直接创建、完成和查看日程
- 显示优化：内容直接显示在单层窗口中，移除套层面板与可见滚动条；紧凑卡片在屏幕或手动尺寸不足时提供翻页，管理子页移除重复返回栏
...
- 行动、今天的初始尺寸均为 320 × 320；日历为 940 × 660，管理为 720 × 380（逻辑像素，随 Windows DPI 缩放，超出屏幕工作区时自动收缩）。
- 行动的待完成、即将开始在有内容时自动展开，空列表默认折叠；行动与今天按实际内容向下增高，内容减少后回到初始方形尺寸。
- 拖动任一边缘或四角可分别调整宽高，手动尺寸会保存，后续内容变化不会覆盖它。点击标题栏“恢复自动尺寸”可恢复默认宽度与内容自适应高度。
- 内容直接铺在单层窗口中，保留 40 像素标题栏，移除内层卡片边框、阴影和可见滚动条。行动、今天在手动缩小或达到屏幕高度上限时用上下翻页访问完整内容；日历和管理的长页面仍可用鼠标滚轮浏览。
- 旧布局迁移到新尺寸，保留位置、固定、显示与置顶状态。
