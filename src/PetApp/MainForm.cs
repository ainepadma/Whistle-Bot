...
public sealed class MainForm : Form
{
    ...
    public MainForm()
    {
        ...
        Size = new Size(LoadSize(), LoadSize());
    ...
    private async void OnLoad(object? sender, EventArgs e)
    {
        ...
        var indexUri = new Uri(Path.Combine(AppContext.BaseDirectory, "wwwroot", "index.html"));
        var args = Environment.GetCommandLineArgs();
        if (args.Length > 1 && args[1].StartsWith("pos=", StringComparison.Ordinal))
        {
            var builder = new UriBuilder(indexUri) { Query = args[1] };
            _web.CoreWebView2.Navigate(builder.Uri.ToString());
        }
        else
        {
            _web.CoreWebView2.Navigate(indexUri.ToString());
        }
        _petColor = LoadColor();
    ...
    private void ApplyPetRegion()
    {
        try
        {
            var scale = Math.Min(Width, Height) / 229f;
    ...
    private static string LoadColor()
    {
        ...
        {
            using var doc = JsonDocument.Parse(File.ReadAllText(ConfigPath));
            return doc.RootElement.GetProperty("color").GetString() ?? "#2f86ed";
        }
        catch
        {
            return "#2f86ed";
