  ...
  if (window.chrome && chrome.webview) {
    chrome.webview.addEventListener('message', e => {
      ...
      if (d.type === 'systemIdle') {
        ...
        if (behavior !== 'systemIdle' && behavior !== 'despawn') setBehavior('typing');
      } else if (d.type === 'interact') {
        interact();
      } else if (d.type === 'toast') {
        showTooltip(d.text || '');
      } else if (d.type === 'size') {
        document.documentElement.style.setProperty('--pet-size', (d.size || 280) + 'px');
      } else if (d.type === 'set-color') {
        document.documentElement.style.setProperty('--pet-color', d.color || '#2f86ed');
      } else if (d.type === 'quit') {
        setBehavior('despawn');
      }
    });
